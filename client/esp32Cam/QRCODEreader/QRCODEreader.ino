...
#include "quirc.h"
/*==========================================*/
int PinTrigger = 14; // Pino usado para disparar os pulsos do sensor
int PinEcho = 15; // pino usado para ler a saida do sensor
float TempoEcho = 0;
...
void loop() {
  ...
  float time = ((TempEcho* VelocidadeSom_mporus)/2)*100;
  if (time > 30){
    if (contador != 0){
      contador--;
      Serial.print("Saiu 1. Atual: ");
      Serial.println(contador);
    }else{
      Serial.println("sub de 0");
    }
  }
...
/* ________________________________________________________________________________ */
void postQRCode(String qrcode) {
  Serial.println("req enviada");
  // Ler dados do QR Code
  String qrCodeData = qrcode; // Substitua por leitura real

  // Configurar requisição HTTP
  WiFiClient client;
  HTTPClient http;
  http.begin(client, "http://192.168.31.112:5000/api/qrcode");
  http.addHeader("Content-Type", "application/json");

  // Criar corpo da requisição
  String jsonData = "{ \"qrcode\": \"" + qrCodeData + "\", \"quantidade\": \"" + contador + "\" }";
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    Serial.println("Usuario sem saldo, negado acesso ");
  }
